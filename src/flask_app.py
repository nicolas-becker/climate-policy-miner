...
def postprocess_results(file_directory, output_df, filename, summary_data=None):
    ...
    if summary_data:
        with open(summary_txt_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Tokens Used: {summary_data['tokens_used']}\n\n")
            ...
            # Fallback: write raw summary if structured sections aren't available
            if not any(sections.values()) and summary_data.get('summary_text'):
            ...
            f.write(f"AI Model: Azure OpenAI GPT-4\n")
...
3. Economy-wide Targets Summary (analyze the economy-wide and net-zero targets):
   ...
   - If no economy-wide targets found, state "No specific economy-wide targets identified"
...
4. Transport Targets Summary (analyze transport sector specific targets):
   ...
   - Convert any CO2 equivalent units to megatonnes (MT CO2eq) for consistency
   - If no transport targets found, state "No specific transport sector targets identified"
...
5. Mitigation Actions Summary (analyze planned mitigation measures):
   ...
   - If no mitigation actions found, state "No specific mitigation actions identified"
...
6. Adaptation Actions Summary (analyze planned adaptation measures):
   ...
   - If no adaptation actions found, state "No specific adaptation actions identified"
